...
class WorkLoggerApp:
    ...
    def build_buttons(self):
        frm = tk.Frame(self.root, bg='#121212')
        frm.pack(fill='x', pady=5)
        for txt, cmd in [("Clock In", self.start_logging), ("Clock Out", self.stop_logging), ("Add Entry", self.add_entry)]:
    ...
    def load_tree(self, cards=None):
        ...
        for tc in (cards or fetch_timecards()):
            ...
            self.tree.insert('', 'end', iid=str(tc.id),
                             values=(dt.date(), dt.time(), datetime.strptime(tc.end_time, '%Y-%m-%d %H:%M:%S').time(), f"{hrs:.2f}", tc.description), tags=(tag,))

    def apply_filter(self):
        frm, to = self.from_date.get_date(), self.to_date.get_date()
        filtered = [tc for tc in fetch_timecards() if frm <= datetime.strptime(tc.start_time, '%Y-%m-%d %H:%M:%S').date() <= to]
    ...
    def edit_entry(self, event):
        ...
        def save():
            ...
            new_v, new_d = int(valid_var.get()), desc_text.get('1.0', 'end-1c')
            try:
            ...
            conn = sqlite3.connect(db_path);
            c = conn.cursor()
            c.execute("UPDATE timecards SET start_time=?, end_time=?, valid=?, description=? WHERE id=?",
    ...
    def update_earned(self):
        # sum up all valid hours
        total_hours = sum(tc.duration_hours()[1] for tc in fetch_timecards() if tc.valid)
        #self.earned_lbl.config(text=f"Gross: ${total * self.rate_per_hour:.2f}")
        gross = total_hours * self.rate_per_hour
        net = gross * 0.80
        # Update both labels
